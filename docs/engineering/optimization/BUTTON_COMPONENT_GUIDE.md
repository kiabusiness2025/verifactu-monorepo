...
```typescript
className =
  'px-6 py-3 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition-colors';
```
...
```typescript
className =
  'flex w-full items-center justify-between py-3 text-left font-medium text-gray-900 hover:text-indigo-600 transition-colors';
```
