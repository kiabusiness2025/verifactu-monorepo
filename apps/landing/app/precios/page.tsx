        ...
        <Container>
          <div className="mx-auto max-w-5xl text-center">
            ...
            <p className="mx-auto mt-5 max-w-3xl text-base leading-7 text-slate-600 sm:text-lg">
              Empieza con control diario y cumplimiento VeriFactu. Sube a Empresa o Pro cuando necesites integración contable, más volumen y una operación más avanzada.
            </p>
          </div>
          ...
          <div className="mx-auto mt-10 grid max-w-6xl gap-4 text-left lg:grid-cols-4">
            {PLAN_LIST.map((plan) => (
              <article key={plan.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{plan.name}</div>
                ...
                <ul className="mt-5 space-y-2 text-sm text-slate-700">
                  ...
                  <li>{plan.hasAccountingIntegration ? 'Integración contable vía API' : 'Sin integración API contable'}</li>
                </ul>
                <div className="mt-6 flex flex-col gap-2">
                  <Link href={`/planes/${plan.id}`} className="inline-flex justify-center rounded-full border border-[#2361d8] px-4 py-2 text-sm font-semibold text-[#2361d8] hover:bg-[#2361d8]/10">
                    Ver detalle
                  </Link>
                  <Link href={getPlanCheckoutHref(plan)} className="inline-flex justify-center rounded-full bg-[#2361d8] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1f55c0]">
          ...
          <div className="mx-auto mt-14 max-w-6xl rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
                <thead>
                  <tr>
                    <th className="border-b border-slate-200 px-4 py-3 font-semibold text-slate-700">Característica</th>
                    {PLAN_LIST.map((plan) => (
                      <th key={plan.id} className="border-b border-slate-200 px-4 py-3 font-semibold text-[#011c67]">{plan.name}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {comparisonRows.map((row) => (
                    <tr key={row.label}>
                      <td className="border-b border-slate-100 px-4 py-3 font-medium text-slate-700">{row.label}</td>
                      {row.values.map((value, index) => (
                        <td key={`${row.label}-${index}`} className="border-b border-slate-100 px-4 py-3 text-slate-600">{value}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="mx-auto mt-14 grid max-w-6xl gap-4 lg:grid-cols-3">
            {featureBlocks.map((block) => (
              <article key={block.title} className="rounded-3xl border border-slate-200 bg-slate-50 p-6">
