...
const SYSTEM_PROMPT = `Eres Isaak, el copiloto de administración de Verifactu Business.
    ...
    const rows = await query<{
      ...
      `SELECT
       ...
       HAVING MAX(al.created_at) IS NULL
          OR MAX(al.created_at) < NOW() - ($1 * INTERVAL '1 day')
  ...
  if (name === 'get_connector_errors') {
    const rows = await query<{
      ...
      `SELECT
       ...
       LEFT JOIN tenant_profiles tp ON tp.tenant_id = ec.tenant_id
       WHERE ec.provider = 'holded'
         AND ec.connection_status IN ('error', 'revoked_api')
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Agentic loop: tool_use → execute → tool_result → …
