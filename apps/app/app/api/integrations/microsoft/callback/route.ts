...
/**
 * API para callback OAuth2 de Microsoft/Outlook
 * GET /api/integrations/microsoft/callback?code=...
 */
export async function GET(req: Request) {
  try {
    ...
    const tokenResponse = await fetch(
      'https://login.microsoftonline.com/common/oauth2/v2.0/token',
      {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: `${process.env.NEXTAUTH_URL}/api/integrations/microsoft/callback`,
    ...
    return NextResponse.redirect(
      `${process.env.NEXTAUTH_URL}/dashboard/settings?tab=integrations&success=microsoft_connected`
    );
  } catch (error) {
