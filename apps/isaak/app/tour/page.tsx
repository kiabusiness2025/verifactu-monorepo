...
// ── Dataset (Nova Gestión demo data) ──────────────────────────────────────────
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-100 text-[10px]">
      {BALANCE.map((row, i) => (
        <div
          key={i}
          className={`flex justify-between px-2.5 py-1 ${
            row.head
              ? 'bg-[#0b2060] font-semibold text-white'
              : i % 2 === 0
...
const ISAAK_SIGNUP = '/auth/signup';
const CYCLE_MS = 7000;
...
export default function TourPage() {
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f5f9ff_0%,#ffffff_40%)] pb-20">
      {/* Hero */}
      <section className="px-4 pb-8 pt-12 text-center">
        <div className="inline-flex items-center gap-2 rounded-full border border-[#2361d8]/15 bg-[#2361d8]/5 px-4 py-1.5 text-xs font-semibold uppercase tracking-widest text-[#2361d8]">
          Isaak en acción
        </div>
        <h1 className="mt-5 text-3xl font-bold tracking-tight text-[#011c67] sm:text-4xl lg:text-5xl">
          8 escenarios reales.
          <br className="hidden sm:block" />
          Datos de empresa vivos.
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-slate-600">
          Isaak conecta con tu Holded, analiza tus datos y genera informes, gráficos y documentos
          exportables — en lenguaje natural, sin fórmulas ni dashboards.
        </p>
        <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <a
            href={ISAAK_SIGNUP}
            className="inline-flex items-center gap-2 rounded-full bg-[#2361d8] px-7 py-3 text-sm font-semibold text-white shadow-md transition hover:bg-[#1f55c0]"
          >
            Prueba 14 días gratis <ArrowRight className="h-4 w-4" />
          </a>
          <Link
            href="/demo"
            className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-7 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
          >
          ...
          {SCENARIOS.map((_, i) => (
            <button
              ...
              className={`rounded-full transition-all duration-300 ${
                i === current ? 'h-2 w-5 bg-[#2361d8]' : 'h-2 w-2 bg-slate-300 hover:bg-slate-400'
              }`}
            />
          ))}
        </div>
      ...
      <section className="mx-auto mt-16 max-w-4xl px-4">
        <div className="rounded-[2rem] border border-[#2361d8]/15 bg-[linear-gradient(135deg,#f0f5ff_0%,#ffffff_100%)] p-8 text-center sm:p-12">
          <h2 className="text-2xl font-bold tracking-tight text-[#011c67] sm:text-3xl">
            ¿Listo para conectar tu empresa?
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-600">
            14 días de prueba gratuita. Sin tarjeta. Sin migrar nada de tu Holded.
          </p>
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <a
              href={ISAAK_SIGNUP}
            ...
            <Link
              href="/conectores"
              className="inline-flex items-center rounded-full border border-slate-300 bg-white px-7 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
            >
              Ver conectores MCP
            </Link>
          </div>
