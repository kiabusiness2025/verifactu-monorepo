...
export async function POST(request: NextRequest) {
  ...
  if (!invoiceHandled) {
    ...
    const [memoryContext, fewShotExamples, bankAccounts] = await Promise.all([
      conversation
        ? getSimpleMemoryContext(
    ...
    const bankingConnected = bankAccounts.length > 0;
    const bankingTotalBalance =
      Math.round(bankAccounts.reduce((s, a) => s + Number(a.balance), 0) * 100) / 100;
    const bankingLine = bankingConnected
      ? `${bankAccounts.length} cuenta(s) conectada(s). Saldo total actual: ${bankingTotalBalance.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €.`
      : null;
