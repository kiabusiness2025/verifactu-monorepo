// PDF export endpoint.
//
// GET /api/isaak/export/pdf?reportType=libro_iva_emitidas&from=YYYY-MM-DD&to=YYYY-MM-DD
//
// Generates a PDF from the Isaak Ledger on-the-fly. Same auth and params as
// the Excel endpoint. tenantId comes from the session.
...
import { buildPdfReport } from '@/app/lib/isaak-pdf-export';
  ...
  try {
    ...
    if (reportType === 'libro_iva_emitidas') {
      ...
      rows = [
        ...
        ['Resultado a ingresar / devolver', resultado.toFixed(2)],
      ];
    ...
    console.error('[Isaak Export PDF] failed', err);
