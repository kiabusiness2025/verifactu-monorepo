// C-B2+ — Exporta el borrador 130 como fichero .130 BOE.

import { NextRequest, NextResponse } from 'next/server';
import { getHoldedSession } from '@/app/lib/holded-session';
import { compute130ForTenant } from '@/app/lib/isaak-modelo-130-repo';
import { serialize130, type Modelo130Context } from '@/app/lib/aeat-formats/130/serializer';
...
export async function POST(req: NextRequest) {
  ...
  let body: { ejercicio?: number; periodo?: string; retencionesAcumuladas?: number; ingresosACuenta?: number };
  try {
  ...
  const computed = await compute130ForTenant({
    ...
    retencionesAcumuladas: typeof body.retencionesAcumuladas === 'number' ? body.retencionesAcumuladas : undefined,
    ingresosACuenta: typeof body.ingresosACuenta === 'number' ? body.ingresosACuenta : undefined,
    persist: false,
  });
  if (!computed.ok) {
