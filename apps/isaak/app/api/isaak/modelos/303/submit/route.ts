// C-B1.b — Confirmar el borrador 303 como presentado.
//
// POST /api/isaak/modelos/303/submit
//   { ejercicio: 2026, periodo: '2T' }
//   → { ok, submissionId, taxReturnId, payloadHash, resultado }
//
// Crea un IsaakAeatSubmission con status='pending_aeat' y promueve el
// IsaakTaxReturn de draft a presented. NO envía SOAP a AEAT (C-B1.c).
    ...
    const httpStatus =
      result.error === 'no_draft' || result.error === 'compute_skipped'
        ...
        : result.error === 'duplicate_submission'
          ? 409
          : result.error === 'tax_return_not_in_draft'
            ? 409
            : 400;
    return NextResponse.json(
