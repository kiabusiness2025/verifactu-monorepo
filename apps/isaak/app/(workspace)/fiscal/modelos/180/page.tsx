...
// C-B7 — UI dedicada del 180 (resumen anual retenciones alquileres).
...
export default function Modelo180Page() {
  ...
  async function computeDraft(persist: boolean) {
    ...
    try {
      const res = await fetch('/api/isaak/modelos/180/draft', {
        method: 'POST',
  ...
  async function submitDraft() {
    if (!confirm(`¿Confirmas presentar el modelo 180 ejercicio ${ejercicio}?`)) return;
    setLoadingSubmit(true);
    setError(null);
    try {
      const res = await fetch('/api/isaak/modelos/180/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Resumen anual del modelo 115. Se presenta en enero del año siguiente. Lista cada
          arrendador con base anual y retención anual practicada.
        </p>
      </div>
        ...
        <div className="rounded-xl border border-green-200 bg-green-50 p-5 space-y-2">
          <div className="flex gap-2 items-center text-green-800">
            ...
            <p className="font-semibold text-sm">
              Modelo 180 ejercicio {submitResult.ejercicio} registrado como presentado
            </p>
          </div>
          <p className="text-xs text-green-700">
            {submitResult.perceptores} arrendadores · Total retenciones:{' '}
      ...
      {result && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Summary label="Arrendadores" value={String(result.perceptores)} />
            <Summary label="Total bases" value={eur(result.totalBase)} />
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-slate-500 text-xs">
                <tr>
                  ...
                  <th className="text-left px-3 py-2">Arrendador</th>
                ...
                {result.lineas.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-3 py-4 text-center text-slate-400 italic">
                      Sin arrendadores con retención en el ejercicio.
                    </td>
                  </tr>
                ) : (
