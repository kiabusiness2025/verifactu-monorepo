...
export function renderDocsPageClaude(baseUrl: string) {
  const readOnlyTools = READ_ONLY_TOOL_NAMES.map(
    (name) => `<li><code>${name}</code> — ${escapeHtml(TOOL_HUMAN_DESCRIPTIONS[name])}</li>`
  ).join('');
  const writeTools = WRITE_TOOL_NAMES.map(
  ...
  return pageTemplateClaude(
    'Claude + Holded Connector Documentation',
    `
      <div class="hero">
        <img src="/claude.svg" alt="Claude" />
        <div>
          <h1>Setup Guide: Claude + Holded</h1>
          <p>Complete documentation to connect your Holded account with Claude and start querying your business data.</p>
        </div>
      </div>

      <h2>4 Steps to Connect</h2>
      <div class="grid">
        <div class="panel">
          <strong>Step 1: Get API Key</strong>
          <p>Go to Holded → Settings → Integrations → API and generate a new API key.</p>
        </div>
        <div class="panel">
          <strong>Step 2: Open Claude.ai</strong>
          <p>Go to claude.ai → Settings → Connectors → Add Custom Connector</p>
        </div>
        <div class="panel">
          <strong>Step 3: Paste MCP URL</strong>
          <p><code>${escapeHtml(`${baseUrl}/mcp`)}</code></p>
        </div>
        <div class="panel">
          <strong>Step 4: Authorize</strong>
          <p>Enter your Holded API key when prompted. Claude discovers OAuth automatically.</p>
        </div>
      </div>

      <h2>Available Tools</h2>
      <p>Most tools are <strong>read-only</strong>. Only <code>create_invoice_draft</code> can write (drafts only, requires your review).</p>

      <h3>Read-only Tools</h3>
      <ul>${readOnlyTools}</ul>

      <h3>Write-capable Tools</h3>
      <ul>${writeTools}</ul>

      <h2>Safety & Permissions</h2>
      <div class="warning">
        <strong>Draft-only invoices</strong>
        <p>The connector creates invoice drafts only. It does not issue, send, delete, or finalize invoices. You must review every draft in Holded.</p>
      </div>
      <ul>
        <li>Most tools are read-only</li>
      ...
      <h2>Troubleshooting</h2>
      <ul>
        <li><strong>Connector won't connect?</strong> Remove and re-add using the same MCP URL.</li>
        <li><strong>Authorization fails?</strong> Verify your Holded API key is valid and has required permissions.</li>
        <li><strong>Icon not updating?</strong> Wait for Google's cache to refresh (usually 24-48 hours).</li>
        <li><strong>Token exchange fails?</strong> Confirm ${baseUrl} environment is correctly set.</li>
      </ul>

      <h2>Questions?</h2>
      <p>Contact us at <a href="mailto:${escapeHtml(SUPPORT_EMAIL)}">${escapeHtml(SUPPORT_EMAIL)}</a></p>
...
export function renderPrivacyPageClaude() {
  return pageTemplateClaude(
    'Privacy Policy — Claude + Holded Connector',
    `
      <h1>Privacy Policy</h1>
      <p>This policy describes how we handle your data when you use the Claude + Holded connector.</p>

      <h2>What Data We Collect</h2>
      <ul>
        <li>Your Holded API key (encrypted and stored securely)</li>
        <li>OAuth session tokens (access token: 1 hour; refresh token: 30 days)</li>
        <li>Business data you query (invoices, contacts, reports — never stored beyond the query)</li>
        <li>Operational logs (for security and support, max 90 days, no conversation content)</li>
      </ul>

      <h2>How We Use It</h2>
      <ul>
        <li>To connect Claude to your Holded account</li>
        <li>To retrieve and display your business data</li>
        <li>To maintain service security and quality</li>
        <li>To comply with legal obligations</li>
      </ul>

      <h2>Your API Key</h2>
      <p>Your Holded API key is:</p>
      <ul>
        <li>Encrypted with AES-256</li>
        <li>Stored in a secure database</li>
        <li>Never shared with Anthropic or third parties</li>
        <li>Revocable at any time (disconnect the connector in Claude)</li>
      </ul>

      <h2>Data Transfer</h2>
      <p>We use Cláusulas Contractuales Tipo (SCC) for transfers to the US (Anthropic). Holded data remains in the EU.</p>

      <h2>Your Rights (GDPR)</h2>
      <ul>
        <li><strong>Access:</strong> Request what data we hold</li>
        <li><strong>Rectification:</strong> Correct any errors</li>
        <li><strong>Deletion:</strong> Remove your data (after disconnect)</li>
        <li><strong>Portability:</strong> Export your data in a readable format</li>
      </ul>

      <p>For privacy requests, email <a href="mailto:${escapeHtml(SUPPORT_EMAIL)}">${escapeHtml(SUPPORT_EMAIL)}</a></p>
...
export function renderDpaPageClaude() {
  return pageTemplateClaude(
    'Data Processing Agreement (DPA) — Claude + Holded',
    `
      <h1>Data Processing Agreement (DPA)</h1>
      <p>This agreement governs how we process your data under GDPR (EU 2016/679).</p>

      <h2>Responsible Party</h2>
      <p><strong>Expert Estudios Profesionales</strong><br/>
      Domain: claude.verifactu.business<br/>
      Email: <a href="mailto:${escapeHtml(SUPPORT_EMAIL)}">${escapeHtml(SUPPORT_EMAIL)}</a><br/>
      Country: Spain (EU)</p>

      <h2>Sub-processors</h2>
      <ul>
        <li><strong>Holded S.L.</strong> (Spain) — API access to your Holded account (GDPR)</li>
        <li><strong>Anthropic, PBC</strong> (USA) — Claude processing (SCC)</li>
        <li><strong>Prisma/Neon/Supabase</strong> (EU/USA) — Session database (SCC/DPA)</li>
        <li><strong>Resend Inc.</strong> (USA) — Transactional emails (SCC)</li>
      </ul>

      <h2>Data Categories</h2>
      <ul>
        <li>Identification data (email, name)</li>
        <li>Holded API key (encrypted)</li>
        <li>Business data queried (read-only)</li>
        <li>Session tokens (limited TTL)</li>
        <li>Activity logs (90 days max)</li>
      </ul>

      <h2>Retention</h2>
      <ul>
        <li><strong>Access tokens:</strong> 1 hour</li>
        <li><strong>Refresh tokens:</strong> 30 days</li>
        <li><strong>User accounts:</strong> Until deleted</li>
      ...
      <ul>
        ...
        <li>AES-256 encryption for stored API keys</li>
        <li>Rate limiting and access controls</li>
        <li>Regular security audits</li>
      </ul>

      <h2>International Transfers</h2>
      <p>Data transfers to the US (Anthropic, Resend) are protected by Cláusulas Contractuales Tipo (SCC) approved by the European Commission (Decision 2021/914).</p>

      <h2>Your GDPR Rights</h2>
      <p>You have the right to:</p>
      <ul>
        <li><strong>Access</strong> your personal data</li>
        <li><strong>Rectify</strong> incorrect information</li>
      ...
      <p>To exercise these rights, email <a href="mailto:${escapeHtml(SUPPORT_EMAIL)}">${escapeHtml(SUPPORT_EMAIL)}</a></p>

      <h2>Data Protection Authority</h2>
      <p>If you have concerns about our data practices, you can lodge a complaint with the Spanish Data Protection Authority (AEPD) at <a href="https://www.aepd.es" target="_blank">www.aepd.es</a></p>
